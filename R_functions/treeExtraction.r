treeExtraction = function(mcc_tree, mostRecentSamplingDatum)
	{
		mcc_tab = matrix(nrow=dim(mcc_tree$edge)[1], ncol=7)
		colnames(mcc_tab) = c("node1","node2","length","startLon","startLat","endLon","endLat") #,"startNodeL","endNodeL","startYear","endYear")
		mcc_tab[,c("node1","node2")] = mcc_tree$edge
		mcc_tab[,c("length")] = mcc_tree$edge.length
		for (i in 1:length(mcc_tree$annotations))
			{
				annotations = mcc_tree$annotations[[i]]
				...
				if (length(index) > 0)
					{
						...
						annotations = mcc_tree$root.annotation
