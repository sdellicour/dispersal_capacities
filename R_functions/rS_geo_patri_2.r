rS_geo_patri_2 = function(tab)
	{
		...
		rS = cor(log(distTree[lower.tri(distTree)]),log(distsGeo[lower.tri(distsGeo)]), method="spearman")
		rS = cor(distTree[lower.tri(distTree)],distsGeo[lower.tri(distsGeo)], method="spearman")
		return(rS)
	}
