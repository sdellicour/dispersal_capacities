spreadFactorsM = function(localTreesDirectory="", nberOfExtractionFiles=1, envVariables=list(), pathModel=1, resistances=list(), avgResistances=list(), fourCells=FALSE, 
								...
								for (t in 1:length(buffer))
									{
										...
										if (usingDiffusionVelocity == TRUE) distances[[t]][,h] = distances[[t]][,h]^2
									}		
							}	else	{
								for (t in 1:nberOfExtractionFiles)
									{
										...
										for (j in 1:nberOfConnections[t])
											{
												...
												if (usingDiffusionVelocity == TRUE) distances[[t]][j,h] = distances[[t]][j,h]^2
											}
										dev.off()
									}
							}
										...
										for (t in 1:length(buffer))
											{
												...
												if (usingDiffusionVelocity == TRUE) distancesSim[[t]][,h] = distancesSim[[t]][,h]^2
											}	
									}		
							}
					}
